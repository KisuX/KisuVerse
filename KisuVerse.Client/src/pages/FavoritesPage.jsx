...
function FavoritesPage() {
  ...
  useEffect(() => {
    async function fetchFavorites() {
      setLoading(true)
      const response = await authFetch('/media/myfavs')
      setLoading(false)
      if (!response.ok) return
      const data = await response.json()
      setMovies(data.map(fromMediaList))
    }
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <h1 className="flex items-center gap-2 text-3xl text-primary" style={{ fontFamily: 'var(--font-display)' }}>
          <IconHeart width={26} height={26} className="fill-current" /> {t('favorites.title')}
        </h1>
        ...
        {movies.length > 0 && (
          ...
          >
            <option value="added">{t('watchlist.sortAdded')}</option>
      ...
      <MovieGrid movies={pageMovies} emptyMessage={t('favorites.empty')} loading={loading} />
