...
function WatchlistPage() {
  ...
  useEffect(() => {
    async function fetchWatchlist() {
      ...
      setMovies(data.map(fromMediaCard))
    }
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <h1 className="flex items-center gap-2 text-3xl text-primary" style={{ fontFamily: 'var(--font-display)' }}>
          <IconBookmark width={26} height={26} className="fill-current" /> {t('watchlist.title')}
        </h1>
      ...
      <MovieGrid movies={pageMovies} emptyMessage={t('watchlist.empty')} loading={loading} />
